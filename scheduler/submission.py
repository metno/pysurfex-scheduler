...
class PBSSubmission(BatchSubmission):
    def __init__(self, task, task_settings, sub="qsub", stat="qstat -j", kill="qdel", prefix="#PBS", db=None):
...
class SlurmSubmission(BatchSubmission):
    def __init__(self, task, task_settings, sub="sbatch", stat="squeue -j", kill="scancel", prefix="#SBATCH", db=None):
        BatchSubmission.__init__(self, task, task_settings, db=db, sub=sub, stat=stat, kill=kill, prefix=prefix)
        name = self.task.ecf_name.split("/")
        self.name = name[-1]
...
class GridEngineSubmission(BatchSubmission):
    def __init__(self, task, task_settings, db=None, sub="qsub", stat="qstat -j", kill="qdel", prefix="#$"):
        BatchSubmission.__init__(self, task, task_settings, db=db, sub=sub, stat=stat, kill=kill, prefix=prefix)
        name = self.task.ecf_name.split("/")
        self.name = name[-1]
